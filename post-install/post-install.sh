...
DEBIAN_FRONTEND=noninteractive sudo apt-get install -y i3 gnome-terminal terminator feh dmenu dunst connman-ui rox-filer hexchat tightvncserver vim-gnome libxss1 libappindicator1 libindicator7 libnspr4 libnss3 libappindicator1 firefox gnome-settings-daemon xautolock i3blocks
...
sudo $(/bin/sh mkdir -p /usr/local/go && curl -Ls https://storage.googleapis.com/golang/go1.5.2.linux-amd64.tar.gz | tar xvzf - -C /usr/local/go --strip-components=1)
